<template>
  <nav
    class="md:hidden bg-white border-gray-200 dark:bg-gray-900 dark:text-white"
  >
    <div
    ...
    >
      ...
      >
        <svg
          ...
          viewBox="0 0 412.452 412.452"
          xml:space="preserve"
        >
          <path
            id="XMLID_47_"
            d="M345.823,241.2c-3.652,0-7.225,0.495-10.652,1.463c-4.233-17.117-19.722-29.844-38.127-29.844
	...
	s-9-4.029-9-9v-3.472c0-4.971,4.029-9,9-9S308.281,247.776,308.281,252.747z"
      ...
      <div class="flex gap-3">
        ...
        >
          ...
          >
            <path
              d="M15 16H9M14.5 9C14.5 7.61929 13.3807 6.5 12 6.5M6 9C6 11.2208 7.2066 13.1599 9 14.1973V18.5C9 19.8807 10.1193 21 11.5 21H12.5C13.8807 21 15 19.8807 15 18.5V14.1973C16.7934 13.1599 18 11.2208 18 9C18 5.68629 15.3137 3 12 3C8.68629 3 6 5.68629 6 9Z"
              stroke="currentColor"
              stroke-width="2"
      ...
      <div class="hidden w-full md:block md:w-auto" id="navbar-default">
        <ul
          class="font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-slate-200 md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700"
        >
          <li v-for="link in navLinks" :key="link.id">
            <RouterLink
              class="block capitalize py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent"
              :to="{ name: link.routeName }"
              :class="{ hidden: link.routeName === route.name }"
              >{{ link.name }}
            </RouterLink>
          </li>
            ...
            <li><RouterLink
              class="block capitalize py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent"
              v-if="!studentsStore.isAuthenticated"
  ...
  <header
    class="hidden md:flex flex-col justify-between items-center pb-10 md:pb-10 md:pt-3 dark:bg-gray-900 dark:text-white"
  >
    <div class="flex gap-3 justify-between flex-row w-full pb-5 px-2">
      <div>
        <p
          class="hover:text-teal-500 text-start capitalize"
          v-if="studentsStore.user.data"
        >
          Hola {{ studentsStore.user.data.name }}
        </p>
      </div>
    ...
    <div
      class="border-2 border-teal-500 p-7"
      :class="{ hidden: route.name !== 'index' }"
    >
      <h1 class="text-center text-9xl uppercase tracking-widest">Get Fit!</h1>
    </div>
    <p
      class="text-4xl -m-6 bg-white px-4 cursive text-teal-500 dark:bg-gray-900"
      :class="{ hidden: route.name !== 'index' }"
    >
      personal trainer
    </p>
  ...
  <nav
    class="hidden md:flex justify-center gap-10 pb-10 dark:bg-gray-900 dark:text-white"
  >
    <RouterLink
      v-for="link in navLinks"
      :key="link.id"
      class="uppercase text-2xl hover:text-teal-500 hover:underline"
      :to="{ name: link.routeName }"
      :class="{
        hidden:
          link.routeName === route.name ||
          (studentsStore.isAuthenticated && link.routeName === 'new-student')
            ? 'hidden'
            : '',
      }"
      >{{ link.name }}
    ...
    <button @click="toggleDark()" type="button" class="hover:text-teal-500">
      <svg
        class="w-5 h-5"
        viewBox="0 0 24 24"
        fill="none"
...
const navLinks = [
  { id: 1, name: "Inicio", routeName: "index" },
  { id: 1, name: "Quienes somos?", routeName: "about" },
  // { id: 3, name: 'servicios',routeName: 'services' },

  { id: 7, name: "contactanos", routeName: "contact" },
  { id: 8, name: "registrate", routeName: "new-student" },
  { id: 6, name: "Blog", routeName: "blog" },
];
